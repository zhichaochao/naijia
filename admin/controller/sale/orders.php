...
class ControllerSaleOrders extends Controller {
	...
	protected function getList() {
		...
		$results = $this->model_sale_order->getOrderspt($filter_order_id);
// print_r($results);exit;
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'view'          => $this->url->link('sale/orders/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, true),
				'edit'          => $this->url->link('sale/orders/edit', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, true)
			);
		}
		...
		$this->response->setOutput($this->load->view('sale/ptorder_list', $data));
	}
	...
	public function getForm() {
		...
		$data['cancel'] = $this->url->link('sale/orders', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['action'] = $this->url->link('sale/orders/orderstatuss', 'token=' . $this->session->data['token'] . $url, true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('sale/ptorder_form', $data));
	}
	public function orderstatuss()
	{
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->model_sale_order->editorderstatuss($this->request->post['order_id'], $this->request->post['order_status_id']);
...
// print_r($order_info);exit();
		if ($order_info) {
			...
			$data['shippingorpick'] = $order_info['shippingorpick'];
...
// print_r($data['products']);exit();
			...
			$this->response->setOutput($this->load->view('sale/ptorder_info', $data));
		} else {
