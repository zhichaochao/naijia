	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', color = '" . $this->db->escape($product['color']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");

				$order_product_id = $this->db->getLastId();
				// print_r($product['option']);
				if ($product['option']['selects']) {
					foreach ($product['option']['selects'] as $option) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_select SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['option_id'] . "', product_option_value_id = '" . (int)$option['option_value_id'] . "', name = '" . $this->db->escape($option['option_name']) . "', `value` = '" . $this->db->escape($option['option_value_name']) . "'");
					// print_r("INSERT INTO " . DB_PREFIX . "order_select SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['option_id'] . "', product_option_value_id = '" . (int)$option['option_value_id'] . "', name = '" . $this->db->escape($option['option_name']) . "', `value` = '" . $this->db->escape($option['option_value_name']) . "'");exit();
				}
				}
		...
		// $this->sendEmail($order_id, $order_status_id);
		// }
// exit();
		// Gift Voucher
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
					...
					$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_select WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "'");
				...
				foreach ($order_product_query->rows as $product) {
					...
					$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_select WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . $product['order_product_id'] . "'");
...
public function addOrderHistoryss($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $shippingNumber = '') {
	...
	public function sendEmail($order_id, $order_status_id){
		...
		// if ($order_info['order_status_id'] == 1 && $order_status_id) {
			...
			$order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
		// print_r("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");exit;
			foreach ($order_product_query->rows as $order_product) {
				// Check if there are any linked downloads
				$product_download_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "product_to_download` WHERE product_id = '" . (int)$order_product['product_id'] . "'");
		...
		// print_r($order_status);exit;
			$subject = sprintf($language->get('text_new_subject'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'), $order_info['order_number']);
		
			// HTML Mail
			$data = array();
		
			$data['title'] = sprintf($language->get('text_new_subject'), $order_info['store_name'], $order_info['order_number']);
			...
			// print_r($data['payment_method']);exit;
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['email'] = $order_info['email'];
			$data['telephone'] = $order_info['telephone'];
			$data['ip'] = $order_info['ip'];
			...
			// Text Mail
			$text  = sprintf($language->get('text_new_greeting'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8')) . "\n\n";
			$text .= $language->get('text_new_order_id') . ' ' . $order_info['order_number'] . "\n";
			$text .= $language->get('text_new_date_added') . ' ' . date($language->get('date_format_short'), strtotime($order_info['date_added'])) . "\n";
			$text .= $language->get('text_new_order_status') . ' ' . $order_status . "\n\n";
			...
			// Admin Alert Mail
			if (in_array('order', (array)$this->config->get('config_mail_alert'))) {
				$subject = sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_info['order_number']);
