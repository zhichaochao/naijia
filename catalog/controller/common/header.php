...
// Analytics
		...
		foreach ($categories as $category) {
			if ($category['top']&&$category['category_id']==67) {
				...
				// Level 1
				$data['categorieson'][] = array(
						'category_id'=>$category['category_id'],
					'name'     => $category['name'],
					'children' => $children_data,
					'column'   => $category['column'] ? $category['column'] : 1,
					'href'     => $this->url->link('product/hotcategory')
				);
			}
		}
		// print_r(	$data['categorieson']);;exit();
		// 搜索历史
		// if($this->customer->isLogged()){
...
$data['action'] = $this->url->link('common/home', '', true);
  ...
  //       if($data['user_name'] == ' ') $data['user_name'] = $this->customer->getEmail();
		...
		$data['aboutus'] = $this->url->link('information/hottoorder');
...
// 
		if ($_SERVER['HTTPS']) {
			 ...
			 $data['mobileurl']='https://m.naijabeautyhair.com'.$_SERVER['REQUEST_URI'];
			 // $data['mobileurl']='https://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
		}else{

			  $data['mobileurl']='http://m.naijabeautyhair.com'.$_SERVER['REQUEST_URI'];
		}
